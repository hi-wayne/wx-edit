...
const selectionEditRules = [
  ...
  "如果设置了文章风格要求，必须把它当作本次改写的统一风格协议：先识别风格要求中的语气、节奏、措辞、段落密度和禁忌，再把这些要求落实到当前结果。",
  "应用文章风格要求时不要只做表面换词；需要让句子长短、表达松紧、情绪强度、口语/书面比例和公众号阅读节奏都尽量贴合。",
  "输出前做一次内部自检：结果是否符合文章风格要求、是否仍在当前选区范围内、是否保留事实和原意；不符合就修正后再返回。",
...
const quickActions = [
  {
    label: "润色",
    instruction: `${selectionEditRules}\n任务：在保留原意和信息量的前提下，把选中内容改得更顺、更准确、更有公众号文章的阅读感。文章风格要求存在时，以它校准语气、节奏和用词；减少口水话和重复表达，保留作者原本语气，不要过度煽情。`
  },
  {
    label: "缩短",
    instruction: `${selectionEditRules}\n任务：压缩选中内容到原文约 50%-70% 长度，保留核心信息、关键判断和必要细节。文章风格要求存在时，删减后仍要保持指定语气和节奏，不要因为压缩变成生硬摘要。删除重复、铺垫、弱连接句，让表达更干净。`
  },
  {
    label: "扩写",
    instruction: `${selectionEditRules}\n任务：围绕选中内容适度扩写到原文约 1.5-2 倍，补足读者理解所需的背景、解释、转折或感受。文章风格要求存在时，扩写新增句子必须贴合该风格，不要只在原文后面机械补充。不要凭空编造具体数据、行程、人物或事件。`
  },
  {
    label: "查错",
    instruction: `${selectionEditRules}\n任务：只修正错别字、病句、标点、语序和明显不自然表达。文章风格要求存在时，只用它判断措辞是否顺手和统一，不要借查错重写成另一篇文章。不要改变原意，不要扩写。`
  }
];
...
const imageActions = [
  {
    label: "生成说明",
    instruction: "只处理当前选中的图片：根据图片、图片 alt 和上下文，生成一句适合微信公众号正文的 figcaption 图片说明。若存在文章风格要求，图片说明的语气、长短和措辞必须贴合该风格。只修改当前图片的 figcaption，不改正文其他内容。"
  },
  {
    label: "按图写段",
    instruction: "参考当前选中的图片和上下文，在图片后面补一小段适合微信公众号阅读的正文。若存在文章风格要求，新增段落必须按该风格写，尤其注意语气、节奏和段落密度。不要改动图片本身，不要新增其他图片。"
  },
  {
    label: "换图建议",
    instruction: "只参考当前选中的图片和文章上下文，给出一个更合适的配图方向，并在当前图片的 figcaption 中写成具体换图建议。若存在文章风格要求，建议文字和图片气质都要贴合该风格。不要改正文其他内容。"
  }
];
...
const aiPromptGuideItems = [
  ...
  })),
  {
    ...
    body: [
      "自定义编辑请求。",
      "如果设置了文章风格要求，必须先判断用户自定义要求和文章风格要求如何同时满足；最终结果要明显体现该风格。",
  ...
  },
  {
    ...
    body: [
      ...
      "如果设置了文章风格要求，新增内容必须按该风格写，并和全文已有语气自然衔接。",
...
function App() {
  ...
  function buildCustomInstruction(userPrompt: string): string {
    ...
    return [
      "自定义编辑请求。",
      "如果设置了文章风格要求，必须把它当作本次处理的写作协议：先识别语气、节奏、措辞、段落密度和禁忌，再把它落实到结果里。",
      contextRule,
      scopeRule,
      "优先满足用户的直接要求；如果要求不完整，按微信公众号正文编辑的常规做最小必要修改。",
      "输出前自检：结果是否符合文章风格要求、是否仍在允许范围内、是否保留事实和原意。",
      "保留事实、数字、时间、地点和作者基本语气，不编造材料。",
      "尽量保留现有 HTML 结构，只在必要时调整段落、强调或列表。",
      mediaRule,
  ...
  function submitInsertRequest() {
    ...
    if (insertTarget === "title") {
      void submitAiRequest(
        [
          ...
          "如果设置了文章风格要求，标题新增文字必须体现该风格的语气和措辞，但仍要短、清楚、事实可靠。",
    ...
    void submitAiRequest(
      [
        "AI 插入请求。只生成要插入到光标位置的新内容，不改写现有正文、标题或摘要。",
        "如果设置了文章风格要求，新增内容必须按该风格写，并和全文已有语气自然衔接；不要只做表面换词。",
        "内容要适合微信公众号手机端阅读，段落短一些，表达自然，不要营销腔。",
        "如果用户要求字数，请尽量贴近字数；如果用户要求列表、标题、图片或其他结构，请按要求生成对应 HTML。",
        "不编造具体事实、数据、人物或时间；需要补充时用通用表述。",
        mediaRule,
        "输出前自检：新增内容是否符合文章风格要求、是否插入在光标位置、是否遵守图片权限。",
        "插入的新区域必须带 data-ai-result=\"true\"。",
        `用户要求：${text}`
      ].join("\n"),
        ...
        <div className="modalOverlay" role="presentation" onMouseDown={() => setShowPromptGuide(false)}>
          <section className="promptGuideModal" role="dialog" aria-modal="true" aria-labelledby="prompt-guide-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="promptGuideIntro">
              <p><strong>文章风格要求：</strong>会作为统一写作协议进入每次 AI 请求，用来约束语气、节奏、措辞、段落密度、情绪强度和表达禁忌；AI 输出前会按它做内部自检。</p>
              <p><strong>边界优先：</strong>风格要求不能覆盖事实准确性、选区范围、标题/正文边界、禁图规则和微信兼容规则。</p>
