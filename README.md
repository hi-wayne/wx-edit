...
- 顶部复制到公众号：优先复制真实富文本 DOM，弹窗提示后可去公众号后台正文区直接粘贴；微信公众号后台可能过滤本地图片，图片建议用导出图文包后上传替换
- 顶部导出：优先选择本地目录导出 `article.wechat.html`、`article.md` 和本地图片资源；浏览器不支持目录选择时导出到 `.wx-editor`
